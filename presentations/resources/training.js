jQuery(document).ready(function(){
	...
	function showTOC(){
		...
		// .html("Agenda");
		jQuery("#pre, #code").height(500).show();
	...
	function loadSample(){
		...
		// if ( !source ) {
		if (jQuery("dd").eq(pos).children().length == 0) {
		...
		var description = jQuery("dd").eq(pos).children("div");
		if (description) {
			jQuery("#description").html(description.html());
		}
		jQuery("#results").empty();

		code.add("#pre").height(500)[0];

		if ( code[0].scrollHeight > 500 )
